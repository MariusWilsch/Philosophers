...
/*   Updated: 2023/04/27 15:37:28 by verdant          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cleanup(t_config *config, int n_philos)
{
	...
	while (i < n_philos)
	{
		if (n_philos != 1 && pthread_join(config->philos_arr[i].thread, NULL) != 0)
 ...
 * @brief Main function
 * 
 * @note To Test intialization of structs // init_test(&config, config.philos_arr);
